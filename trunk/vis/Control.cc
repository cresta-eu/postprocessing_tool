...

{
  namespace vis
  {
    ...
    void Control::initLayers()
    {
      ...
      myCutter = new cutplane::CutPlane(*latticeData, screen, viewpoint, visSettings, propertyCache);
    ...
    void Control::Render(unsigned long startIteration)
    {
          ...
          && (visSettings.mStressType == lb::ShearStress || visSettings.mode == VisSettings::WALLANDSTREAKLINES))
      {
        streak = myStreaker->Render();
         ...
         PixelSet<cutplane::CutPlanePixel>* cut =myCutter->Render();

      localResultsByStartIt.insert(std::pair<unsigned long, Rendering>(startIteration, Rendering(glyph, ray, streak, cut)));
