...
namespace hemelb
{
  namespace vis
  {
    ResultPixel::ResultPixel(const BasicPixel* glyph) :
      BasicPixel(glyph->GetI(), glyph->GetJ()), hasGlyph(true), normalRayPixel(NULL), streakPixel(NULL), cutPlanePixel(NULL)
    {

    }

    ResultPixel::ResultPixel(const raytracer::RayDataNormal* ray) :
      BasicPixel(ray->GetI(), ray->GetJ()), hasGlyph(false), normalRayPixel(ray), streakPixel(NULL),cutPlanePixel(NULL)
    {

    }

    ResultPixel::ResultPixel(const streaklinedrawer::StreakPixel* streak) :
      BasicPixel(streak->GetI(), streak->GetJ()), hasGlyph(false), normalRayPixel(NULL), streakPixel(streak),cutPlanePixel(NULL)
    {

    }

    ResultPixel::ResultPixel(const cutplane::CutPlanePixel* cut) :
      BasicPixel(cut->GetI(), cut->GetJ()), hasGlyph(false), normalRayPixel(NULL),streakPixel(NULL), cutPlanePixel(cut)
    {
    ...
    void ResultPixel::Combine(const ResultPixel& other)
    {
      ...
      if (other.cutPlanePixel != NULL)
      {
        cutPlanePixel = other.cutPlanePixel;
      }
    }
      ...
      *pixel_index = (i << (2 * bits_per_char)) + j;
       ...
       if (cutPlanePixel!=NULL){ 
        ...
        {rgb_data[ii]=0;
         ...
         float vel =(float)iDomainStats.velocity_threshold_max_inv * cutPlanePixel->getVelocity();

         float c[3];
         PickColour(vel, c);

        
         MakePixelColour(int(255.0F * c[0]),
                         int(255.0F * c[1]),
                         int(255.0F * c[2]),
                         rgb_data);
         return;
